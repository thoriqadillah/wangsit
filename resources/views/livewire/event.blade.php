    ...
    <div class="grid grid-col-1 lg:grid-cols-3 2xl:grid-cols-4 sm:grid-cols-2 mt-8 gap-x-8 gap-y-10 justify-items-center">
        ...
        <div class="w-80 h-96 sm:w-full md:w-[360px] lg:w-full shadow">
            ...
            <div class="px-4">
                ...
                <a href="/event/{{ $event->slug }}/daftar{{ $filter === 'aktif' ? '' : '/pengumuman' }}" class="block w-full rounded-md shadow-md bg-mainColor text-center text-white py-3 mt-6">{{ $filter === 'aktif' ? 'DAFTAR' : 'PENGUMUMAN' }}</a>
            </div>
        </div>
        @endforeach
    </div>
